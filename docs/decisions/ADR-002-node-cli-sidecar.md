...
The base image (`ghcr.io/kirodotdev/kirocrew:stable`) ships Python and common CLI tools
but intentionally omits language runtimes to keep the image minimal.

## Decision

Inject Node.js 20 LTS using the same sidecar pattern as `docker-cli` (ADR-001):

1. A `node-cli` init service based on `node:20-slim` copies `node` and the global module tree into a named volume `node-bin`, then recreates the `npm`, `npx`, and `corepack` launcher symlinks inside that volume.
